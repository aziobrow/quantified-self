...
const styleRemainingCalories = (remainingCalories) =>  {
  if (remainingCalories.innerHTML >= 0) {
    remainingCalories.parentElement.style.backgroundColor = "green"
  } else {
    remainingCalories.parentElement.style.backgroundColor = "red"
  }
}
...
$(".add-to-meal").on("click", function(event) {
  var meal = event.target.innerHTML
  console.log(meal)
})
