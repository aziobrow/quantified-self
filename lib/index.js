...
const mealFile = require('./meals.js')
...
const addNewFood = (event) => {
  ...
  if (!(foodFile.getFoodNames()).includes(newName)){
    foodFile.makeRequest('POST', '', body)
    .then((response) => helperFile.handleResponse(response))
    .catch((error) => console.error({error}))
    $("#new-food-form").trigger('reset')
    setTimeout(getFoods, 300)
  }
}
...
const getMealFoods = (mealId) => {
  let numId = parseInt(mealId)
  $('[data-meal=' + numId + '] > .meal-food-info').remove()
  fetch(`${helperFile.url}api/v1/meals/${numId}/foods`)
  .then(response => helperFile.handleResponse(response))
...
const deleteMealFood = (event) => {
  ...
  .catch((error) => console.error({error}))
  setTimeout(getMealFoods(mealId), 300)
}
