...
const filterFoods = () =>  {
  ...
  console.log(page)
    ...
    $('#available-food-table').filter((search) => {
      let input = $('#diary-search-term').val().toLowerCase()
      let rows = $('#available-food-table > tbody > tr')
      for (let i=1; i < rows.length; i++) {
        let td = rows[i].getElementsByTagName("td")[0]
        if (td) {
          if (td.innerHTML.toLowerCase().indexOf(input) > -1) {
...
module.exports = {getFoods, filterFoods, renderFood, url, handleResponse, getPage}
